...
- UI 只管理表单草稿、选择、路由、滚动、窗口等展示状态；关闭全部 UI 后 daemon 与 agent 的后台状态机仍可继续（是否继续由 agent 的驻留策略决定，UI 不自建托盘或常驻逻辑）。
- 进程退出分两种且不得混用：完全退出走 `system.shutdown`（agent 先关停 daemon，连接以 `service-quit` 关闭，客户端不得重拉）；SIGTERM 只退出收到信号的进程。
